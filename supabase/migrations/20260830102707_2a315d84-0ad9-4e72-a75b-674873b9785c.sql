create or replace function erp.seed_demo_bom(p_fg uuid, p_rm uuid, p_site uuid)
returns text language plpgsql security definer set search_path to '' as $$
declare
  v_tenant uuid := erp.require_tenant_id();
  v_actor uuid := erp.current_principal_id();
  v_uom uuid; v_bom uuid;
begin
  if p_fg = p_rm then
    return 'A bill of materials needs two different items.';
  end if;
  if exists (select 1 from erp.bom b where b.tenant_id = v_tenant and b.item_id = p_fg) then
    return 'A bill of materials already exists.';
  end if;
  select stock_uom_id into v_uom from erp.item where tenant_id = v_tenant and id = p_rm;
  ...
  values (v_tenant, 'DEMO-BOM', p_fg, p_site, 1, 'Demo bill of materials', 1,
          'active'::erp.config_version_status, current_date - 30, v_actor, v_actor)
  returning id into v_bom;
  insert into erp.bom_line (tenant_id, bom_id, seq, component_item_id, quantity, uom_id,
                            created_by, updated_by)
  values (v_tenant, v_bom, 10, p_rm, 2, v_uom, v_actor, v_actor);
  return 'Created a demo bill of materials.';
end $$;

revoke all on function erp.seed_demo_bom(uuid, uuid, uuid) from public, anon, authenticated;
...
declare
  ...
  v_bulk uuid; v_prog text; v_n integer;
begin
  ...
  begin
    v_po := erp.create_document('purchase_order', v_entity, v_site, v_supplier, current_date, v_ccy);
  ...
  if v_po is not null then
    begin
      v_receipt := erp.create_document('goods_receipt', v_entity, v_site, v_supplier, current_date, v_ccy);
  ...
  begin
    v_so := erp.create_document('sales_order', v_entity, v_site, v_customer, current_date, v_ccy);

...
begin
  ...
  -- The site must belong to a company that keeps a ledger, otherwise every
  -- document is refused for reporting in a currency no ledger accepts.
  ...
  begin
    if v_fg <> v_rm and not exists (select 1 from erp.bom b
                                     where b.tenant_id = v_tenant and b.item_id = v_fg) then

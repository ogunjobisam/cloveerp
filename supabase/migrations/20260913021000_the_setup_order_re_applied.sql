-- The setup walkthrough register, re-applied.
--
-- 20260913020000 was edited after it was pushed, three times, and a migration
-- is written once. The first version gave the audit screen a blurb one
-- character short of the register's own check, so the file refused at that
-- statement on every environment that tried it and applied nowhere. The
-- second fixed the word. The third took out three refusal registrations for
-- codes erp.refusal_report() does not read, which its own last line refused.
-- The fourth corrected what a review found: the cost-centre dimension is
-- stored as COST_CENTRE and three evidence branches compared lower case, and
-- three steps named a permission other than the one their door authorises.
--
-- The rule does not ask whether an earlier version reached anywhere: it asks
-- for a new migration that re-applies the definition, and a line in
-- supabase/ci/migrations_edited.txt naming it. This is that migration. It
-- re-applies the setup order, the step register and the evidence function
-- exactly as the edited file states them, so an environment that had somehow
-- kept an earlier version would hold what one that had not holds.

...
-- seeded month looked right and every real organisation's did not.
--
-- The ledger is not wrong about it, and the obvious complaint turns out not to
-- be the one to make. The receivable is settled, the control account is down,
-- and since 20260918500000 the ageing reads what is owed rather than the
-- document's face value, so it carries nothing for an invoice that owes
-- nothing. A paid invoice does not sit on the ageing for ever. That was
-- checked rather than assumed, and it is why this change says nothing about
-- the ageing being wrong.
--
-- What is wrong is the document. It says issued: on the customer's copy, on
-- the document screen, to the person picking up the phone to chase it, and to
-- every list of open invoices that selects on state. It still offers Record
-- payment, and it still offers Credit. The v1 Definition of Done asks for an
-- order-to-cash journey that ends with AR clear (O2C-01) and one that part-pays
-- (O2C-06); the money reached the right place in both and the document
-- finished neither.
--
-- ── Where it belongs ─────────────────────────────────────────────────────────
--
...
declare
  ...
  v_owed bigint; v_scr bigint; v_bal bigint; v_settled bigint;
  v_s1 text;
...
begin
  begin
    ...
    -- Asked twice, of the two things that have to agree. A receipt applied
    ...
    select coalesce(sum(si.debit_minor - si.credit_minor - coalesce(si.settled_minor, 0)), 0),
      ...
      into v_bal, v_settled
      from erp.subledger_item si
     where si.tenant_id = rb.tenant_id and si.document_id = v_inv
       and si.control_kind = 'receivable';

    v_cases := v_cases + 1;
    case_name := 'and the subledger owes nothing on it: the whole invoice is settled, and the ageing reads the same nil';
    passed := v_state is null and v_owed = 0 and v_bal = 0 and v_settled = v_gross;
    detail := coalesce(v_state, format('%s outstanding on the ageing; the invoice''s own rows owe %s, with %s of %s settled against them',
                                       v_owed, v_bal, v_settled, v_gross));
    return next;

    -- ── 3. And the ageing has stopped carrying it ───────────────────────────
    v_cases := v_cases + 1;
    case_name := 'and the ageing agrees with the document: neither the report nor the receivables screen carries it';

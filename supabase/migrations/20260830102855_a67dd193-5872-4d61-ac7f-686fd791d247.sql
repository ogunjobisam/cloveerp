...
declare
  ...
  v_recv uuid; v_prog text; v_n integer;
begin
  ...
  -- Re-running should be safe: the history is built once and then left alone.
  if exists (select 1 from erp.document d
              where d.tenant_id = v_tenant and d.their_reference = 'DEMO-SEED') then
    return jsonb_build_object('ok', true, 'site', v_site, 'notes',
      jsonb_build_array('Demonstration history already exists for this company; nothing was duplicated.'));
  end if;
  ...
  -- Materials are issued from where they actually are, which is goods-in until
  -- somebody completes the putaway task raised above.
  begin

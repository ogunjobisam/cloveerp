-- The forecast already names the supplier it would buy from. Naming is not
-- enough to raise an order against them, so the row now carries the supplier's
-- own record as well, and the buying screen can act on the line it is reading.
...
comment on function erp.stock_forecast_lines is
  ...
  'and what is on order — with days of cover, the reorder-by date, a suggested '
  'quantity and the supplier to buy it from, per product per site.';

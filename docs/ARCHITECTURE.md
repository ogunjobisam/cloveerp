...
| **Build**              | Every migration applied to an empty database on every push, then <!-- count:catalogue_checks -->297<!-- /count --> catalogue checks, three rehearsals against a stub endpoint, and the checks that every door and every screen string has a home.                              |
...
<!-- count:migrations -->453<!-- /count --> migrations and
<!-- count:sql_lines -->289346<!-- /count --> lines of SQL.
...
| `erp_test`    | <!-- count:suites -->204<!-- /count --> suites                                                    | The harness. Suites build their own organisations, attack them, and roll them back       |
...
check the build can call — <!-- count:assertions -->111<!-- /count --> structural
assertions, <!-- count:suites -->204<!-- /count --> adversarial suites, the
...
<!-- count:diagnostic_checks -->118<!-- /count --> registered diagnostics
(<!-- count:diagnostic_checks_in_ci -->97<!-- /count --> of them also in CI) and

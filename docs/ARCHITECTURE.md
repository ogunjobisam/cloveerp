...
| **Interface**          | An application over a curated API of <!-- count:doors -->690<!-- /count --> doors; a scan-first device client; a platform console; a status page.                                                                                                                             |
| **Build**              | Every migration applied on every push — to a base a previous build proved, and nightly to an empty database — then <!-- count:catalogue_checks -->345<!-- /count --> catalogue checks, three rehearsals against a stub endpoint, and the checks that every door and every screen string has a home.                              |
...
<!-- count:migrations -->534<!-- /count --> migrations and
<!-- count:sql_lines -->332133<!-- /count --> lines of SQL.
...
- **`erp_meta.public_write_allowance`** — <!-- count:write_allowances -->506<!-- /count --> entries. The doors permitted to be volatile, each naming the gate it reaches. A volatile `public.erp_*` function that is not listed fails the build; so does one whose gate no longer authorises.
...
| `erp_test`    | <!-- count:suites -->244<!-- /count --> suites                                                    | The harness. Suites build their own organisations, attack them, and roll them back       |
| `public`      | <!-- count:doors -->690<!-- /count --> functions                                                  | The only surface PostgREST exposes                                                       |
...
<!-- count:en_strings -->5009<!-- /count --> English strings, a German core pack
of <!-- count:de_strings -->699<!-- /count -->, a tenant's own terms under
...
assertions, <!-- count:suites -->244<!-- /count --> adversarial suites, the
...
<!-- count:diagnostic_checks -->126<!-- /count --> registered diagnostics

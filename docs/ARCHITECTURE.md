...
| **Interface**          | An application over a curated API of <!-- count:doors -->685<!-- /count --> doors; a scan-first device client; a platform console; a status page.                                                                                                                             |
| **Build**              | Every migration applied to an empty database on every push, then <!-- count:catalogue_checks -->307<!-- /count --> catalogue checks, three rehearsals against a stub endpoint, and the checks that every door and every screen string has a home.                              |
...
<!-- count:migrations -->468<!-- /count --> migrations and
<!-- count:sql_lines -->297101<!-- /count --> lines of SQL.
...
- **`erp_meta.security_definer_allowance`** — <!-- count:definer_allowances -->203<!-- /count --> entries. A `SECURITY DEFINER` function runs as the owner, who bypasses row-level security. Every one in the product schemas is listed with the reason it needs the privilege.
- **`erp_meta.public_write_allowance`** — <!-- count:write_allowances -->501<!-- /count --> entries. The doors permitted to be volatile, each naming the gate it reaches. A volatile `public.erp_*` function that is not listed fails the build; so does one whose gate no longer authorises.
...
| `erp_test`    | <!-- count:suites -->211<!-- /count --> suites                                                    | The harness. Suites build their own organisations, attack them, and roll them back       |
| `public`      | <!-- count:doors -->685<!-- /count --> functions                                                  | The only surface PostgREST exposes                                                       |
...
<!-- count:en_strings -->4893<!-- /count --> English strings, a German core pack
...
assertions, <!-- count:suites -->211<!-- /count --> adversarial suites, the

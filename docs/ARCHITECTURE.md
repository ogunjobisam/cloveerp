...
| **Interface**          | An application over a curated API of <!-- count:doors -->663<!-- /count --> doors; a scan-first device client; a platform console; a status page.                                                                                                                             |
| **Build**              | Every migration applied to an empty database on every push, then <!-- count:catalogue_checks -->247<!-- /count --> catalogue checks, three rehearsals against a stub endpoint, and the checks that every door and every screen string has a home.                              |

Concretely: <!-- count:erp_tables -->251<!-- /count --> tenant tables,
<!-- count:ref_tables -->76<!-- /count --> product-content tables,
<!-- count:meta_tables -->68<!-- /count --> platform tables,
<!-- count:enums -->83<!-- /count --> enumerated types,
<!-- count:policies -->367<!-- /count --> row-security policies and
<!-- count:triggers -->817<!-- /count --> triggers — of which the policies and
most of the triggers are _generated_, not written — in
<!-- count:migrations -->389<!-- /count --> migrations and
<!-- count:sql_lines -->245480<!-- /count --> lines of SQL.
...
- **`erp_meta.security_definer_allowance`** — <!-- count:definer_allowances -->194<!-- /count --> entries. A `SECURITY DEFINER` function runs as the owner, who bypasses row-level security. Every one in the product schemas is listed with the reason it needs the privilege.
- **`erp_meta.public_write_allowance`** — <!-- count:write_allowances -->485<!-- /count --> entries. The doors permitted to be volatile, each naming the gate it reaches. A volatile `public.erp_*` function that is not listed fails the build; so does one whose gate no longer authorises.
...
| ------------- | ------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| `erp`         | <!-- count:erp_tables -->251<!-- /count --> tables, <!-- count:erp_views -->15<!-- /count --> views | Tenant data and the engines                                                              |
| `erp_ref`     | <!-- count:ref_tables -->76<!-- /count --> tables                                                 | Product content — what the product knows, identical for every tenant                     |
| `erp_meta`    | <!-- count:meta_tables -->68<!-- /count --> tables                                                | Platform metadata: the registers, the allow-lists, the exemptions, incidents, releases   |
...
| `erp_test`    | <!-- count:suites -->161<!-- /count --> suites                                                    | The harness. Suites build their own organisations, attack them, and roll them back       |
| `public`      | <!-- count:doors -->663<!-- /count --> functions                                                  | The only surface PostgREST exposes                                                       |
...
<!-- count:en_strings -->4325<!-- /count --> English strings, a German core pack
of <!-- count:de_strings -->682<!-- /count -->, a tenant's own terms under
...
check the build can call — <!-- count:assertions -->101<!-- /count --> structural
assertions, <!-- count:suites -->161<!-- /count --> adversarial suites, the
...
<!-- count:diagnostic_checks -->108<!-- /count --> registered diagnostics
(<!-- count:diagnostic_checks_in_ci -->89<!-- /count --> of them also in CI) and

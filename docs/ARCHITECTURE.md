...
| **Interface**          | An application over a curated API of <!-- count:doors -->681<!-- /count --> doors; a scan-first device client; a platform console; a status page.                                                                                                                             |
...
<!-- count:sql_lines -->278570<!-- /count --> lines of SQL.
...
- **`erp_meta.public_write_allowance`** — <!-- count:write_allowances -->497<!-- /count --> entries. The doors permitted to be volatile, each naming the gate it reaches. A volatile `public.erp_*` function that is not listed fails the build; so does one whose gate no longer authorises.
...
| `public`      | <!-- count:doors -->681<!-- /count --> functions                                                  | The only surface PostgREST exposes                                                       |
...
<!-- count:en_strings -->4622<!-- /count --> English strings, a German core pack

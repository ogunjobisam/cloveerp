...
| **Interface**          | An application over a curated API of <!-- count:doors -->671<!-- /count --> doors; a scan-first device client; a platform console; a status page.                                                                                                                             |
| **Build**              | Every migration applied to an empty database on every push, then <!-- count:catalogue_checks -->268<!-- /count --> catalogue checks, three rehearsals against a stub endpoint, and the checks that every door and every screen string has a home.                              |
...
<!-- count:triggers -->820<!-- /count --> triggers — of which the policies and
most of the triggers are _generated_, not written — in
<!-- count:migrations -->412<!-- /count --> migrations and
<!-- count:sql_lines -->259092<!-- /count --> lines of SQL.
...
- **`erp_meta.public_write_allowance`** — <!-- count:write_allowances -->489<!-- /count --> entries. The doors permitted to be volatile, each naming the gate it reaches. A volatile `public.erp_*` function that is not listed fails the build; so does one whose gate no longer authorises.
...
| `erp_test`    | <!-- count:suites -->180<!-- /count --> suites                                                    | The harness. Suites build their own organisations, attack them, and roll them back       |
| `public`      | <!-- count:doors -->671<!-- /count --> functions                                                  | The only surface PostgREST exposes                                                       |
...
<!-- count:en_strings -->4433<!-- /count --> English strings, a German core pack
of <!-- count:de_strings -->684<!-- /count -->, a tenant's own terms under
...
check the build can call — <!-- count:assertions -->104<!-- /count --> structural
assertions, <!-- count:suites -->180<!-- /count --> adversarial suites, the
...
<!-- count:diagnostic_checks -->110<!-- /count --> registered diagnostics
(<!-- count:diagnostic_checks_in_ci -->90<!-- /count --> of them also in CI) and

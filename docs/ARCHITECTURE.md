...
| **Interface**          | An application over a curated API of <!-- count:doors -->682<!-- /count --> doors; a scan-first device client; a platform console; a status page.                                                                                                                             |
| **Build**              | Every migration applied to an empty database on every push, then <!-- count:catalogue_checks -->291<!-- /count --> catalogue checks, three rehearsals against a stub endpoint, and the checks that every door and every screen string has a home.                              |

Concretely: <!-- count:erp_tables -->251<!-- /count --> tenant tables,
<!-- count:ref_tables -->77<!-- /count --> product-content tables,
<!-- count:meta_tables -->73<!-- /count --> platform tables,
...
<!-- count:migrations -->449<!-- /count --> migrations and
<!-- count:sql_lines -->286619<!-- /count --> lines of SQL.
...
- **`erp_meta.security_definer_allowance`** — <!-- count:definer_allowances -->201<!-- /count --> entries. A `SECURITY DEFINER` function runs as the owner, who bypasses row-level security. Every one in the product schemas is listed with the reason it needs the privilege.
- **`erp_meta.public_write_allowance`** — <!-- count:write_allowances -->498<!-- /count --> entries. The doors permitted to be volatile, each naming the gate it reaches. A volatile `public.erp_*` function that is not listed fails the build; so does one whose gate no longer authorises.
...
| `erp_ref`     | <!-- count:ref_tables -->77<!-- /count --> tables                                                 | Product content — what the product knows, identical for every tenant                     |
| `erp_meta`    | <!-- count:meta_tables -->73<!-- /count --> tables                                                | Platform metadata: the registers, the allow-lists, the exemptions, incidents, releases   |
...
| `erp_test`    | <!-- count:suites -->200<!-- /count --> suites                                                    | The harness. Suites build their own organisations, attack them, and roll them back       |
| `public`      | <!-- count:doors -->682<!-- /count --> functions                                                  | The only surface PostgREST exposes                                                       |
...
<!-- count:en_strings -->4681<!-- /count --> English strings, a German core pack
of <!-- count:de_strings -->691<!-- /count -->, a tenant's own terms under
...
check the build can call — <!-- count:assertions -->109<!-- /count --> structural
assertions, <!-- count:suites -->200<!-- /count --> adversarial suites, the
...
<!-- count:diagnostic_checks -->116<!-- /count --> registered diagnostics
(<!-- count:diagnostic_checks_in_ci -->95<!-- /count --> of them also in CI) and

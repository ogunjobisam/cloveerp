...
<!-- count:sql_lines -->246924<!-- /count --> lines of SQL.
...
- **`erp_meta.security_definer_allowance`** — <!-- count:definer_allowances -->196<!-- /count --> entries. A `SECURITY DEFINER` function runs as the owner, who bypasses row-level security. Every one in the product schemas is listed with the reason it needs the privilege.
...
| `erp_test`    | <!-- count:suites -->162<!-- /count --> suites                                                    | The harness. Suites build their own organisations, attack them, and roll them back       |
...
<!-- count:en_strings -->4338<!-- /count --> English strings, a German core pack
...
assertions, <!-- count:suites -->162<!-- /count --> adversarial suites, the

...
| **Interface**          | An application over a curated API of <!-- count:doors -->661<!-- /count --> doors; a scan-first device client; a platform console; a status page.                                                                                                                             |
| **Build**              | Every migration applied to an empty database on every push, then <!-- count:catalogue_checks -->244<!-- /count --> catalogue checks, three rehearsals against a stub endpoint, and the checks that every door and every screen string has a home.                              |
...
<!-- count:migrations -->387<!-- /count --> migrations and
<!-- count:sql_lines -->244443<!-- /count --> lines of SQL.
...
- **`erp_meta.public_write_allowance`** — <!-- count:write_allowances -->483<!-- /count --> entries. The doors permitted to be volatile, each naming the gate it reaches. A volatile `public.erp_*` function that is not listed fails the build; so does one whose gate no longer authorises.
...
| `erp_test`    | <!-- count:suites -->159<!-- /count --> suites                                                    | The harness. Suites build their own organisations, attack them, and roll them back       |
| `public`      | <!-- count:doors -->661<!-- /count --> functions                                                  | The only surface PostgREST exposes                                                       |
...
<!-- count:en_strings -->4322<!-- /count --> English strings, a German core pack
...
assertions, <!-- count:suites -->159<!-- /count --> adversarial suites, the

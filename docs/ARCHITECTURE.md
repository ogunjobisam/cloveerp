...
| **Interface**          | An application over a curated API of <!-- count:doors -->627<!-- /count --> doors; a scan-first device client; a platform console; a status page.                                                                                                                             |
| **Build**              | Every migration applied to an empty database on every push, then <!-- count:catalogue_checks -->221<!-- /count --> catalogue checks, three rehearsals against a stub endpoint, and the checks that every door and every screen string has a home.                              |
...
<!-- count:migrations -->362<!-- /count --> migrations and
<!-- count:sql_lines -->218257<!-- /count --> lines of SQL.
...
- **`erp_meta.public_write_allowance`** — <!-- count:write_allowances -->454<!-- /count --> entries. The doors permitted to be volatile, each naming the gate it reaches. A volatile `public.erp_*` function that is not listed fails the build; so does one whose gate no longer authorises.
...
| `erp_test`    | <!-- count:suites -->138<!-- /count --> suites                                                    | The harness. Suites build their own organisations, attack them, and roll them back       |
| `public`      | <!-- count:doors -->627<!-- /count --> functions                                                  | The only surface PostgREST exposes                                                       |
...
<!-- count:en_strings -->3828<!-- /count --> English strings, a German core pack
...
assertions, <!-- count:suites -->138<!-- /count --> adversarial suites, the

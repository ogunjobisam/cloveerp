...
| **Build**              | Every migration applied to an empty database on every push, then <!-- count:catalogue_checks -->295<!-- /count --> catalogue checks, three rehearsals against a stub endpoint, and the checks that every door and every screen string has a home.                              |
...
<!-- count:migrations -->452<!-- /count --> migrations and
<!-- count:sql_lines -->288734<!-- /count --> lines of SQL.
...
| `erp_test`    | <!-- count:suites -->203<!-- /count --> suites                                                    | The harness. Suites build their own organisations, attack them, and roll them back       |
...
check the build can call — <!-- count:assertions -->110<!-- /count --> structural
assertions, <!-- count:suites -->203<!-- /count --> adversarial suites, the
...
<!-- count:diagnostic_checks -->117<!-- /count --> registered diagnostics
(<!-- count:diagnostic_checks_in_ci -->96<!-- /count --> of them also in CI) and

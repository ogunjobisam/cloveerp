...
| **Build**              | Every migration applied to an empty database on every push, then <!-- count:catalogue_checks -->281<!-- /count --> catalogue checks, three rehearsals against a stub endpoint, and the checks that every door and every screen string has a home.                              |
...
<!-- count:migrations -->435<!-- /count --> migrations and
<!-- count:sql_lines -->278397<!-- /count --> lines of SQL.
...
| `erp_test`    | <!-- count:suites -->193<!-- /count --> suites                                                    | The harness. Suites build their own organisations, attack them, and roll them back       |
...
<!-- count:en_strings -->4652<!-- /count --> English strings, a German core pack
...
check the build can call — <!-- count:assertions -->106<!-- /count --> structural
assertions, <!-- count:suites -->193<!-- /count --> adversarial suites, the
...
<!-- count:diagnostic_checks -->112<!-- /count --> registered diagnostics
(<!-- count:diagnostic_checks_in_ci -->92<!-- /count --> of them also in CI) and

...
| **Interface**          | An application over a curated API of <!-- count:doors -->649<!-- /count --> doors; a scan-first device client; a platform console; a status page.                                                                                                                             |
...
<!-- count:migrations -->377<!-- /count --> migrations and
<!-- count:sql_lines -->235489<!-- /count --> lines of SQL.
...
- **`erp_meta.security_definer_allowance`** — <!-- count:definer_allowances -->183<!-- /count --> entries. A `SECURITY DEFINER` function runs as the owner, who bypasses row-level security. Every one in the product schemas is listed with the reason it needs the privilege.
- **`erp_meta.public_write_allowance`** — <!-- count:write_allowances -->473<!-- /count --> entries. The doors permitted to be volatile, each naming the gate it reaches. A volatile `public.erp_*` function that is not listed fails the build; so does one whose gate no longer authorises.
...
| `erp_test`    | <!-- count:suites -->151<!-- /count --> suites                                                    | The harness. Suites build their own organisations, attack them, and roll them back       |
| `public`      | <!-- count:doors -->649<!-- /count --> functions                                                  | The only surface PostgREST exposes                                                       |
...
<!-- count:en_strings -->4154<!-- /count --> English strings, a German core pack
...
assertions, <!-- count:suites -->151<!-- /count --> adversarial suites, the

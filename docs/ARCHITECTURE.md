...
| **Build**              | Every migration applied to an empty database on every push, then <!-- count:catalogue_checks -->204<!-- /count --> catalogue checks, three rehearsals against a stub endpoint, and the checks that every door and every screen string has a home.                              |
...
<!-- count:migrations -->348<!-- /count --> migrations and
<!-- count:sql_lines -->203749<!-- /count --> lines of SQL.
...
- **`erp_meta.security_definer_allowance`** — <!-- count:definer_allowances -->174<!-- /count --> entries. A `SECURITY DEFINER` function runs as the owner, who bypasses row-level security. Every one in the product schemas is listed with the reason it needs the privilege.
...
| `erp_test`    | <!-- count:suites -->122<!-- /count --> suites                                                    | The harness. Suites build their own organisations, attack them, and roll them back       |
...
<!-- count:en_strings -->3730<!-- /count --> English strings, a German core pack
...
assertions, <!-- count:suites -->122<!-- /count --> adversarial suites, the

...
| **Build**              | Every migration applied to an empty database on every push, then <!-- count:catalogue_checks -->286<!-- /count --> catalogue checks, three rehearsals against a stub endpoint, and the checks that every door and every screen string has a home.                              |

Concretely: <!-- count:erp_tables -->251<!-- /count --> tenant tables,
<!-- count:ref_tables -->77<!-- /count --> product-content tables,
<!-- count:meta_tables -->71<!-- /count --> platform tables,
...
<!-- count:migrations -->442<!-- /count --> migrations and
<!-- count:sql_lines -->282603<!-- /count --> lines of SQL.
...
- **`erp_meta.security_definer_allowance`** — <!-- count:definer_allowances -->200<!-- /count --> entries. A `SECURITY DEFINER` function runs as the owner, who bypasses row-level security. Every one in the product schemas is listed with the reason it needs the privilege.
...
| `erp_meta`    | <!-- count:meta_tables -->71<!-- /count --> tables                                                | Platform metadata: the registers, the allow-lists, the exemptions, incidents, releases   |
...
| `erp_test`    | <!-- count:suites -->196<!-- /count --> suites                                                    | The harness. Suites build their own organisations, attack them, and roll them back       |
...
<!-- count:en_strings -->4667<!-- /count --> English strings, a German core pack
...
check the build can call — <!-- count:assertions -->108<!-- /count --> structural
assertions, <!-- count:suites -->196<!-- /count --> adversarial suites, the
...
<!-- count:diagnostic_checks -->115<!-- /count --> registered diagnostics
(<!-- count:diagnostic_checks_in_ci -->94<!-- /count --> of them also in CI) and

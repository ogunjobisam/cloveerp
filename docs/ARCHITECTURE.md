...
| **Interface**          | An application over a curated API of <!-- count:doors -->658<!-- /count --> doors; a scan-first device client; a platform console; a status page.                                                                                                                             |
| **Build**              | Every migration applied to an empty database on every push, then <!-- count:catalogue_checks -->242<!-- /count --> catalogue checks, three rehearsals against a stub endpoint, and the checks that every door and every screen string has a home.                              |
...
<!-- count:migrations -->384<!-- /count --> migrations and
<!-- count:sql_lines -->241768<!-- /count --> lines of SQL.
...
- **`erp_meta.security_definer_allowance`** — <!-- count:definer_allowances -->192<!-- /count --> entries. A `SECURITY DEFINER` function runs as the owner, who bypasses row-level security. Every one in the product schemas is listed with the reason it needs the privilege.
- **`erp_meta.public_write_allowance`** — <!-- count:write_allowances -->482<!-- /count --> entries. The doors permitted to be volatile, each naming the gate it reaches. A volatile `public.erp_*` function that is not listed fails the build; so does one whose gate no longer authorises.
...
| `erp_test`    | <!-- count:suites -->157<!-- /count --> suites                                                    | The harness. Suites build their own organisations, attack them, and roll them back       |
| `public`      | <!-- count:doors -->658<!-- /count --> functions                                                  | The only surface PostgREST exposes                                                       |
...
<!-- count:en_strings -->4302<!-- /count --> English strings, a German core pack
of <!-- count:de_strings -->677<!-- /count -->, a tenant's own terms under
...
check the build can call — <!-- count:assertions -->100<!-- /count --> structural
assertions, <!-- count:suites -->157<!-- /count --> adversarial suites, the
...
<!-- count:diagnostic_checks -->107<!-- /count --> registered diagnostics
(<!-- count:diagnostic_checks_in_ci -->88<!-- /count --> of them also in CI) and

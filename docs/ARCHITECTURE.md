...
| **Build**              | Every migration applied to an empty database on every push, then <!-- count:catalogue_checks -->203<!-- /count --> catalogue checks, three rehearsals against a stub endpoint, and the checks that every door and every screen string has a home.                              |
...
<!-- count:migrations -->346<!-- /count --> migrations and
<!-- count:sql_lines -->199412<!-- /count --> lines of SQL.
...
- **`erp_meta.security_definer_allowance`** — <!-- count:definer_allowances -->173<!-- /count --> entries. A `SECURITY DEFINER` function runs as the owner, who bypasses row-level security. Every one in the product schemas is listed with the reason it needs the privilege.
- **`erp_meta.public_write_allowance`** — <!-- count:write_allowances -->445<!-- /count --> entries. The doors permitted to be volatile, each naming the gate it reaches. A volatile `public.erp_*` function that is not listed fails the build; so does one whose gate no longer authorises.
...
| `erp_ref`     | <!-- count:ref_tables -->74<!-- /count --> tables                                                 | Product content — what the product knows, identical for every tenant                     |
...
| `erp_test`    | <!-- count:suites -->121<!-- /count --> suites                                                    | The harness. Suites build their own organisations, attack them, and roll them back       |
...
assertions, <!-- count:suites -->121<!-- /count --> adversarial suites, the
...
<!-- count:diagnostic_checks -->104<!-- /count --> registered diagnostics
(<!-- count:diagnostic_checks_in_ci -->85<!-- /count --> of them also in CI) and

...
| **Interface**          | An application over a curated API of <!-- count:doors -->629<!-- /count --> doors; a scan-first device client; a platform console; a status page.                                                                                                                             |
| **Build**              | Every migration applied to an empty database on every push, then <!-- count:catalogue_checks -->223<!-- /count --> catalogue checks, three rehearsals against a stub endpoint, and the checks that every door and every screen string has a home.                              |

Concretely: <!-- count:erp_tables -->250<!-- /count --> tenant tables,
<!-- count:ref_tables -->76<!-- /count --> product-content tables,
<!-- count:meta_tables -->65<!-- /count --> platform tables,
<!-- count:enums -->83<!-- /count --> enumerated types,
<!-- count:policies -->366<!-- /count --> row-security policies and
<!-- count:triggers -->812<!-- /count --> triggers — of which the policies and
most of the triggers are _generated_, not written — in
<!-- count:migrations -->364<!-- /count --> migrations and
<!-- count:sql_lines -->222523<!-- /count --> lines of SQL.
...
<!-- count:product_decision_bindings -->79<!-- /count --> assertions and suites
...
- **`erp_meta.security_definer_allowance`** — <!-- count:definer_allowances -->178<!-- /count --> entries. A `SECURITY DEFINER` function runs as the owner, who bypasses row-level security. Every one in the product schemas is listed with the reason it needs the privilege.
- **`erp_meta.public_write_allowance`** — <!-- count:write_allowances -->456<!-- /count --> entries. The doors permitted to be volatile, each naming the gate it reaches. A volatile `public.erp_*` function that is not listed fails the build; so does one whose gate no longer authorises.
- **`erp_meta.check_run_exemption`** — the catalogue checks CI cannot run without an argument, each naming what drives it instead.
- **`erp_meta.api_only_door`** — <!-- count:api_only_doors -->21<!-- /count --> doors no screen names, each with the caller it exists for (the worker, the build, the device client, an integration, the platform) and <!-- count:doors_pending_screen -->4<!-- /count --> waiting for their screen with the path recorded.
...
| `erp_meta`    | <!-- count:meta_tables -->65<!-- /count --> tables                                                | Platform metadata: the registers, the allow-lists, the exemptions, incidents, releases   |
...
| `erp_test`    | <!-- count:suites -->140<!-- /count --> suites                                                    | The harness. Suites build their own organisations, attack them, and roll them back       |
| `public`      | <!-- count:doors -->629<!-- /count --> functions                                                  | The only surface PostgREST exposes                                                       |
...
<!-- count:en_strings -->3898<!-- /count --> English strings, a German core pack
of <!-- count:de_strings -->575<!-- /count -->, a tenant's own terms under
...
check the build can call — <!-- count:assertions -->98<!-- /count --> structural
assertions, <!-- count:suites -->140<!-- /count --> adversarial suites, the
...
`supabase/ci/app_permissions.sh` does the same for the other bare string the
screens carry: every permission code `hasPermission()` is given must be in
`erp_ref.permission`, because a code the catalogue does not hold is in no
session's list and so hides its control from everybody at once — the quietest
failure the application has, and one no permission suite can see.
...
<!-- count:diagnostic_checks -->105<!-- /count --> registered diagnostics
(<!-- count:diagnostic_checks_in_ci -->86<!-- /count --> of them also in CI) and

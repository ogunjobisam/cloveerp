...
| **Interface**          | An application over a curated API of <!-- count:doors -->667<!-- /count --> doors; a scan-first device client; a platform console; a status page.                                                                                                                             |
| **Build**              | Every migration applied to an empty database on every push, then <!-- count:catalogue_checks -->261<!-- /count --> catalogue checks, three rehearsals against a stub endpoint, and the checks that every door and every screen string has a home.                              |

Concretely: <!-- count:erp_tables -->251<!-- /count --> tenant tables,
<!-- count:ref_tables -->76<!-- /count --> product-content tables,
<!-- count:meta_tables -->70<!-- /count --> platform tables,
...
<!-- count:migrations -->406<!-- /count --> migrations and
<!-- count:sql_lines -->254720<!-- /count --> lines of SQL.
...
- **`erp_meta.public_write_allowance`** — <!-- count:write_allowances -->488<!-- /count --> entries. The doors permitted to be volatile, each naming the gate it reaches. A volatile `public.erp_*` function that is not listed fails the build; so does one whose gate no longer authorises.
...
| `erp_meta`    | <!-- count:meta_tables -->70<!-- /count --> tables                                                | Platform metadata: the registers, the allow-lists, the exemptions, incidents, releases   |
...
| `erp_test`    | <!-- count:suites -->174<!-- /count --> suites                                                    | The harness. Suites build their own organisations, attack them, and roll them back       |
| `public`      | <!-- count:doors -->667<!-- /count --> functions                                                  | The only surface PostgREST exposes                                                       |
...
<!-- count:en_strings -->4366<!-- /count --> English strings, a German core pack
...
check the build can call — <!-- count:assertions -->103<!-- /count --> structural
assertions, <!-- count:suites -->174<!-- /count --> adversarial suites, the

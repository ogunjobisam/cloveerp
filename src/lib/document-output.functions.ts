...
import { renderSalesInvoicePdf, sha256Hex } from "./pdf/invoice-pdf";
...
export const documentOutput = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<DocumentOutputResult> => {
    ...
    const swept = await rpc("erp_purge_expired_document_previews");
    ...
    if (data.action === "preview") {
      ...
      const contract = await rpc("erp_sales_invoice_contract", { p_document_id: data.documentId });
      if (contract.error) refuse(contract.error.message);

      const session = await rpc("erp_session");
      const tenant = String(session.data?.tenant_id ?? "");
      if (!tenant) refuse("No organisation is in scope.");

      const bytes = await renderSalesInvoicePdf(contract.data, { watermark: "PREVIEW" });
      ...
      const recorded = await rpc("erp_record_document_preview", {
    ...
    if (data.action === "issue" || data.action === "amend") {
      ...
      const reserved =
        data.action === "issue"
          ? await rpc("erp_issue_sales_invoice", {
              p_document_id: data.documentId,
              p_template_version_id: data.templateVersionId ?? null,
            })
          : await rpc("erp_amend_sales_invoice", {
      ...
      const issues = await rpc("erp_document_issues", { p_document_id: null, p_limit: 200 });
      if (issues.error) refuse(issues.error.message);
      const row = (issues.data as Array<Record<string, unknown>>).find(
        (i) => i["document_issue_id"] === issueId,
      );
    ...
    const reprint = await rpc("erp_reprint_document_issue", {
      p_document_issue_id: data.documentIssueId,

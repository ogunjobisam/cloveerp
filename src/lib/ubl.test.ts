...
const contract: BillingContract = {
  ...
  payment: { meansCode: "30", accountId: "GB82WEST12345698765432", accountName: "Clove Foods Limited" },
  lines: [{ id: "1", quantity: "2", unitCode: "EA", name: "Cloves £", price: "10.00", netAmount: "20.00", taxCategoryCode: "S", taxRate: "20" }],
  taxes: [{ categoryCode: "S", rate: "20", taxableAmount: "20.00", taxAmount: "4.00" }],
  totals: { lineExtension: "20.00", taxExclusive: "20.00", taxInclusive: "24.00", payable: "24.00" },
};

describe("Peppol BIS Billing UBL", () => {
  test("declares and escapes a sales invoice", () => {
    const output = serializeBillingContract(contract);
    expect(output).toContain('<Invoice xmlns="urn:oasis:names:specification:ubl:schema:xsd:Invoice-2"');
    expect(output).toContain(`<cbc:CustomizationID>${PEPPOL_BILLING_CUSTOMIZATION_ID}</cbc:CustomizationID>`);
  ...
  test("uses the credit-note root and requires the original invoice", () => {
    ...
    expect(output).toContain('<CreditNote xmlns="urn:oasis:names:specification:ubl:schema:xsd:CreditNote-2"');
